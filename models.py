...
class brokerage_calc(models.Model):
	...
	VALUES=[('intraday','Equity Intraday'),('delivery','Equity Delivery'),('futures','F&O Futures'),('options','F&O Options')]
...
# input values fields
	...
	def calc_turnover_x(self):
		...
		if self.select_mode == 'intraday':
			intraday = eval(config_param_env.get_param('equity_intraday'))
			int_brokerage = intraday.get('int_brokerage')
			raw_int_brokerage = to * int_brokerage/100
			self.brokerage = raw_int_brokerage if raw_int_brokerage < 20 else 20
			print '\n---',self.brokerage,'---\n'

			#STT Function
			int_stt=intraday.get('int_stt')
			self.stt_total = sell_to * int_stt/100 
			print '\n---',self.stt_total,'---stt_total\n'
			#Transaction Charge
			int_bse_transaction = intraday.get('int_bse_transaction')
			int_nse_transaction = intraday.get('int_nse_transaction')
			if self.tax_select=='bse':
				self.total_txn_charge = (int_bse_transaction if buy_to else 0.0)  +  (int_bse_transaction if sell_to else 0.0)
			elif self.tax_select=='nse':
				self.total_txn_charge= to * (int_nse_transaction/100)
			print '\n---',self.total_txn_charge,'---total_txn_charge\n'

			#gst fucntion
			int_gst = intraday.get('int_gst')
			self.gst= (self.brokerage + self.total_txn_charge) * int_gst/100
			print '\n---',self.gst,'---gst\n'
			#SEBI Charge
			int_sebi_charges=intraday.get('int_sebi')
